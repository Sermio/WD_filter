/// Human specialization tree — **fuente de verdad**: `Worldshift/data/db/items/humansspecs.dt`.
/// Tribes / Aliens: `mutantsspecs.dt`, `aliensspecs.dt` (misma forma de `item` + `stats`).
/// Iconos / posición en atlas: `techgrid.lua` + `spec_tree_icons`.
///
/// Lógica interna del juego: dos filas de rejilla (A/B y C/D). Patrón visual en app: 2-3-2-3.
class HumanSpecTreeNode {
  const HumanSpecTreeNode({
    required this.repo,
    required this.title,
  ...
  /// One entry per investable rank (slash-separated stats in source data).
  final List<String> rankBonuses;

  /// Valores por rango tal cual en `humansspecs.dt` (stats de ese tramo).
  final List<String> rankStatSnippets;

  /// Número de estrellas / rangos comprables en el juego (= longitud de [rankBonuses]).
...
///
/// Fila inferior, orden **lineal** del juego: Precision → First-Aid Mastery → Barrier Armor
/// → Overtraining → Power Surge → fila C [SPECC1, SPECD2], fila D [SPECD1, SPECC2, SPECD3].
